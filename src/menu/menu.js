(function () {
  ...
  function initMatrixExpandedSuiteCards() {
    ...
    // Never create the suite twice.
    if (document.getElementById("matrix-celestar-suite-marker")) return;

    var marker = document.createElement("div");
    marker.id = "matrix-celestar-suite-marker";
    ...
    function renderOptions(id) {
      ...
      var html = '<div class="mod-description">Matrix implementation of this expanded mod.</div>';
      ...
      Object.keys(cfg).forEach(function (key) {
        ...
        if (isBool(v)) {
          html +=
            ...
            '<div class="opt-toggle">' +
            '<input type="checkbox" id="' + esc(safeId) + '" data-mxs-path="' + esc(full) + '"' +
            (v ? ' checked' : '') + '>' +
            '<label for="' + esc(safeId) + '"></label>' +
            '</div></div>';
        } else if (isNum(v)) {
          var lower = key.toLowerCase();
          var min = lower.indexOf("opacity") !== -1 ? 0.1 : (key === "scale" ? 0.5 : 0);
          var max = lower.indexOf("opacity") !== -1 ? 1 :
            (lower.indexOf("size") !== -1 ? 128 :
            ((key === "x" || key === "y") ? 1000 : 10));
          ...
          html +=
            ...
            '<label>' + esc(labelize(key)) + '</label>' +
            '<div class="setting-inline">' +
            '<input type="range" id="' + esc(safeId) + '" data-mxs-path="' + esc(full) +
            '" min="' + min + '" max="' + max + '" step="' + step + '" value="' + v + '">' +
            '<div class="range-val" id="' + esc(safeId.replace(/^mxs-/, "mxsv-")) + '">' + escVal(v) + '</div>' +
            '</div></div>';
          ...
          if (isColor) {
            html +=
              '<div class="setting-row">' +
              '<label>' + esc(labelize(key)) + '</label>' +
              '<input type="color" id="' + esc(safeId) + '" data-mxs-path="' + esc(full) +
              '" value="' + esc(v.slice(0, 7)) + '" class="matrix-suite-color-input">' +
              '</div>';
          } else if (lower.indexOf("keybind") !== -1) {
            html +=
              '<div class="setting-row">' +
              '<label>' + esc(labelize(key)) + '</label>' +
              '<div class="keybind-box" id="' + esc(safeId) + '" data-mxs-path="' + esc(full) + '" tabindex="0">' +
              escVal(v) +
              '</div></div>';
          } else {
            html +=
              '<div class="setting-row">' +
              '<label>' + esc(labelize(key)) + '</label>' +
              '<input class="text-box" type="text" id="' + esc(safeId) + '" data-mxs-path="' + esc(full) + '" value="' +
              esc(v) + '">' +
              '</div>';
          }
        }
    ...
    function bindOptions(id) {
      ...
      qq('#matrix-options-body [data-mxs-path]').forEach(function (el) {
        ...
        if (el.type === 'color') {
          el.oninput = function () {
            suite.set(path, el.value);
          };
          return;
        }

        if (el.classList.contains('keybind-box')) {
          var listening = false;
          var handler = null;
    ...
    suite.listMods().forEach(function (mod) {
      ...
      toggle.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        suite.set(id + '.enabled', !suite.get(id + '.enabled'));
        sync();
      });
      ...
      options.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        if (!optPanel || !optBody || !optTitle) return;
