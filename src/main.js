...
function downloadToFile(url, destination) {
    return new Promise((resolve, reject) => {
        const request = https.get(url, {
            ...
            if (response.statusCode !== 200) {
                response.resume();
                return reject(new Error("Matrix installer download failed with HTTP " + response.statusCode));
            }
        ...
        request.setTimeout(120000, () => {
            request.destroy(new Error("Matrix installer download timed out."));
        });
...
async function installGithubUpdate(remote) {
    ...
    const expectedHash = String(remote.sha256 || "").trim().toLowerCase();

    if (!expectedHash) {
        updateInstalling = false;
        throw new Error("latest.json does not contain sha256.");
    }

    fs.rmSync(updateDir, { recursive: true, force: true });
    fs.mkdirSync(updateDir, { recursive: true });

    await downloadToFile(remote.installerUrl || GITHUB_AUTO_UPDATE.installerUrl, installerPath);
    ...
    if (actualHash !== expectedHash) {
        ...
        updateInstalling = false;
        throw new Error("Matrix installer SHA-256 verification failed.");
    }

    const currentPid = process.pid;
    const targetExe = app.getPath("exe");
...
try { Remove-Item '${escaped(updateDir)}' -Recurse -Force -ErrorAction SilentlyContinue } catch {}
`;
...
async function checkGithubAutoUpdate({ install = true } = {}) {
    ...
    try {
        ...
        const remote = await fetchJson(GITHUB_AUTO_UPDATE.metadataUrl);
        const remoteBuild = String(remote.buildId || "").trim();

        if (!remoteBuild) throw new Error("latest.json does not contain buildId.");
        if (remoteBuild === String(local.buildId || "")) return false;
        if (remote.version && remote.version === local.version && !remoteBuild) return false;

        console.log("[Matrix] GitHub update available:", remote.version || remoteBuild);
...
window.__mfSettings.keystrokesTextPress = ${JSON.stringify(settings["keystrokes.textPressColor"] ?? "#000000")};
    ...
    const matrixCelestarSuiteJs = fs.readFileSync(path.join(__dirname, "features/matrix-celestar-suite.js"), "utf8");
    const matrixCelestarSuiteCss = fs.readFileSync(path.join(__dirname, "menu/matrix-celestar-suite.css"), "utf8");
    ...
    // Matrix Expanded Mod Suite: independent implementations of the expanded mod set.
    try {
        await wc.insertCSS(matrixCelestarSuiteCss);
    } catch (e) {
        console.error("[Matrix] Expanded Mod Suite CSS failed:", e.message);
    }
    try {
        await wc.executeJavaScript(matrixCelestarSuiteJs);
    } catch (e) {
        console.error("[Matrix] Expanded Mod Suite JS failed:", e.message);
    }
